...
const getDefaultFeatureTypes = () =>
  DEFAULT_FEATURE_TYPES.map((featureType) => ({ ...featureType }));
...
const normalizeImageScalingEntry = (entry = {}, channels = []) => {
  const selectedChannel =
    entry?.selectedChannel != null ? String(entry.selectedChannel) : "";
  const patchIds = Array.isArray(entry?.patchIds)
    ? entry.patchIds.map(String)
    : [];
  ...
  const channelVisibility = {
    ...defaultVisibility,
    ...Object.fromEntries(
      Object.entries(entry?.channelVisibility || {}).map(([key, value]) => [
        String(key),
        Boolean(value),
      ]),
    ),
  ...
  const channelScales = Object.fromEntries(
    Object.entries(entry?.channelScales || {}).map(([key, value]) => [
      String(key),
      sanitizeChannelScale(value),
    ]),
  );
  ...
  const channelBounds = Object.fromEntries(
    Object.entries(entry?.channelBounds || {}).flatMap(([key, value]) => {
...
const normalizeDatasetNormalization = (entry = {}, channels = []) => {
  const selectedChannel =
    entry?.selectedChannel != null ? String(entry.selectedChannel) : "";
  const channelScales = Object.fromEntries(
    Object.entries(entry?.channelScales || {}).map(([key, value]) => [
...
const AnnotationTab = ({ preferredSelectedDatasets = [] }) => {
  ...
  const [viewMode, setViewMode] = useState(VIEW_MODE_PATCHES);
  ...
  const [datasetNormalization, setDatasetNormalization] = useState({
    selectedChannel: "",
    channelScales: {},
  });
  ...
  const imagesById = useMemo(
    () =>
      Object.fromEntries(
        datasetImages.map((image) => [String(image.id), image]),
      ),
    [datasetImages],
  );
  const selectedPatch = useMemo(
    () =>
      patches.find((patch) => String(patch.id) === String(selectedPatchId)) ||
      null,
  ...
  const viewerPatch = useMemo(() => {
    ...
    return String(selectedPatch.imageId) === String(selectedImage.id)
      ? selectedPatch
      : null;
  ...
  const resetAnnotationEditor = () => {
    ...
    setDatasetNormalization({ selectedChannel: "", channelScales: {} });
  ...
  const loadAnnotationSets = useCallback(
    async (nextDatasetId, preferredAnnotationSetId = null) => {
      ...
      try {
        ...
        const preferredExists =
          preferredAnnotationSetId &&
          sets.some(
            (annotationSet) =>
              String(annotationSet.id) === String(preferredAnnotationSetId),
          );
    ...
    const loadDatasetImages = async () => {
      ...
      try {
        ...
        for (let page = 1; page <= 50; page += 1) {
          ...
          const freshImages = pageImages.filter(
            (image) => !seenImageIds.has(String(image.id)),
          );
  ...
  useEffect(() => {
    ...
    const fetchAnnotationSet = async () => {
      const selectedSummary = annotationSets.find(
        (annotationSet) =>
          String(annotationSet.id) === String(selectedAnnotationSetId),
      );
      if (
        !selectedSummary ||
        String(selectedSummary.datasetId) !== String(datasetId)
      ) {
        return;
      }
      ...
      setLoadingAnns(true);
      try {
        const data = await fetchMapAnnotations(
          datasetId,
        ...
        setFeatureTypes(
          data.featureTypes?.length
            ? data.featureTypes
            : getDefaultFeatureTypes(),
        );
        ...
        const nextDatasetNormalization = normalizeDatasetNormalization(
          data.selectedChannel != null || data.channelScales
            ? {
        ...
        if (!cancelled) {
          ...
          toaster.show({
            message: "Failed to load annotation set",
            intent: "danger",
          });
        }
      } finally {
  ...
  const updateImageScalingEntry = useCallback(
    (imageId, updater) => {
      if (imageId == null) {
        return;
      }
      ...
      setImageScalingsByImageId((currentEntries) => {
        ...
        const baseEntry = normalizeImageScalingEntry(
          currentEntries[key],
          channels,
        );
        const nextEntry =
          typeof updater === "function" ? updater(baseEntry) : updater;
        return {
          ...currentEntries,
  ...
  useEffect(() => {
    ...
    if (
      !selectedPatchId ||
      String(activePatch.id) !== String(selectedPatchId)
    ) {
    ...
    if (patchImage) {
      setSelectedImage((current) =>
        String(current?.id) === String(patchImage.id) ? current : patchImage,
      );
    } else {
      setSelectedImage((current) => {
  ...
  const loadChannels = useCallback(async (imageId) => {
    try {
      ...
      setImageScalingsByImageId((currentEntries) => ({
        ...currentEntries,
        [String(imageId)]: normalizeImageScalingEntry(
          currentEntries[String(imageId)],
          data.channels || [],
        ),
      }));
      setImageMeta({
      ...
      setDatasetImages((currentImages) =>
  ...
  const currentImageAnnotations = selectedImage
    ? datasetAnnotations.filter(
        (annotation) => String(annotation.imageId) === String(selectedImage.id),
      )
    : [];
  const currentPatchAnnotations = selectedPatch
    ? currentImageAnnotations.filter(
        (annotation) =>
          String(annotation.patchId || "") === String(selectedPatch.id),
      )
    : [];
  const currentViewerAnnotations =
    viewMode === VIEW_MODE_PATCHES
      ? currentPatchAnnotations
  ...
  const currentDatasetNormalization = useMemo(
    () =>
      normalizeDatasetNormalization(
        {
          selectedChannel:
            datasetNormalization.selectedChannel ||
            selectedAnnotationSummary?.selectedChannel,
          channelScales: Object.keys(datasetNormalization.channelScales || {})
            .length
            ? datasetNormalization.channelScales
            : selectedAnnotationSummary?.channelScales || {},
        },
        channels,
      ),
    [channels, datasetNormalization, selectedAnnotationSummary],
  );
  const annotationSetChannelOptions = useMemo(() => {
    if (channels.length) {
    ...
    return Object.keys(currentDatasetNormalization.channelScales || {})
  ...
  const currentImageScaling = useMemo(
    () =>
      normalizeImageScalingEntry(
        currentScalingImageId
          ? imageScalingsByImageId[currentScalingImageId]
          : {},
        channels,
      ),
  ...
  const selectedAnnotationChannelScale = sanitizeChannelScale(
    currentDatasetNormalization.channelScales[selectedAnnotationChannel],
  );

  const handleAnnotationSetChannelScaleChange = useCallback(
    (channelIndex, field, value) => {
      setDatasetNormalization((currentNormalization) => {
        const normalizedCurrent = normalizeDatasetNormalization(
          currentNormalization,
          channels,
        );
        const key = String(channelIndex);
        const previous = sanitizeChannelScale(
          normalizedCurrent.channelScales[key],
        );
  ...
  const handleSelectedAnnotationChannelChange = useCallback(
    (value) => {
      setDatasetNormalization((currentNormalization) => {
        const normalizedCurrent = normalizeDatasetNormalization(
          currentNormalization,
  ...
  const handleCurrentImageChannelVisibilityChange = useCallback(
    (nextVisibility) => {
  ...
  const handleCurrentImageChannelScalesChange = useCallback(
    (nextScales) => {
      if (currentScalingImageId == null) {
        return;
      }
  ...
  const handleCurrentImageChannelBoundsChange = useCallback(
    (nextChannelBounds) => {
      if (currentScalingImageId == null) {
        return;
      }
  ...
  const handleImageAnnotationsChange = (nextAnnotations) => {
    ...
    const otherAnnotations =
      viewMode === VIEW_MODE_PATCHES && selectedPatch
        ? datasetAnnotations.filter(
            (annotation) =>
              !(
                String(annotation.imageId) === selectedImageId &&
                String(annotation.patchId || "") === String(selectedPatch.id)
              ),
          )
        : datasetAnnotations.filter(
    ...
    const normalizedAnnotations = nextAnnotations.map((annotation) => ({
      ...
      patchId:
        viewMode === VIEW_MODE_PATCHES && selectedPatch
          ? String(selectedPatch.id)
          : annotation.patchId
            ? String(annotation.patchId)
            : null,
    }));
  ...
  const handleRemovePatch = (patchToRemove) => {
    ...
    const remainingPatches = patches.filter(
      (patch) => String(patch.id) !== patchId,
    );
    setPatches(remainingPatches);
    setDatasetAnnotations((currentAnnotations) =>
  ...
  const handleAddPatch = () => {
    const nextPatchWidth = Math.max(
      1,
      Math.round(Number(patchWidth) || DEFAULT_PATCH_SIZE),
    );
    const nextPatchHeight = Math.max(
      1,
      Math.round(Number(patchHeight) || DEFAULT_PATCH_SIZE),
    );
    ...
    const minPatchCount = Math.min(
      ...candidates.map(({ image }) => imagePatchCounts[String(image.id)] || 0),
    );
    const preferredImages = candidates.filter(
      ({ image }) =>
        (imagePatchCounts[String(image.id)] || 0) === minPatchCount,
    );
    const target =
      preferredImages[Math.floor(Math.random() * preferredImages.length)];
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const previousAnnotationSetId = selectedAnnotationSetId
        ? String(selectedAnnotationSetId)
        : null;
      const imagesWithPatches = Array.from(
        new Set(patches.map((patch) => String(patch.imageId)).filter(Boolean)),
      );
      const relevantScalingImageIds = imagesWithPatches.length
        ? imagesWithPatches
        : Array.from(
            new Set(
              datasetImages.map((image) => String(image.id)).filter(Boolean),
            ),
          );
      const payload = {
        version: "2.0",
        ...
        selectedChannel:
          selectedAnnotationChannel !== ""
            ? String(selectedAnnotationChannel)
            : null,
        channelScales: Object.fromEntries(
          Object.entries(currentDatasetNormalization.channelScales).map(
            ([key, value]) => [String(key), sanitizeChannelScale(value)],
          ),
        ),
        imageScalings: relevantScalingImageIds.map((imageId) => {
          const entry = normalizeImageScalingEntry(
            imageScalingsByImageId[String(imageId)],
            channels,
          );
          return {
            ...
            channelBounds: Object.fromEntries(
              Object.entries(entry.channelBounds || {}).map(([key, value]) => [
                String(key),
                {
        ...
        annotations: datasetAnnotations.map((annotation) => ({
          ...annotation,
          imageId:
            annotation.imageId != null
              ? String(annotation.imageId)
              : annotation.imageId,
          patchId:
            annotation.patchId != null ? String(annotation.patchId) : null,
        })),
        featureTypes,
      };

      const response = await saveMapAnnotation(
        datasetId,
        payload,
      ...
      setAnnotationSets((currentAnnotationSets) => {
        const filteredSets = currentAnnotationSets.filter((annotationSet) => {
          ...
          return (
            annotationSetId !== nextAnnotationSetId &&
            annotationSetId !== previousAnnotationSetId
          );
        });
      ...
      toaster.show({
        message: selectedAnnotationSetId
          ? "Annotation set updated"
      ...
      toaster.show({
        message: "Failed to save annotation set",
        intent: "danger",
      });
    } finally {
      setSaving(false);
        ...
        <div className="w-[24rem] flex flex-col gap-4 overflow-y-auto min-h-0 pr-1 shrink-0 h-full max-h-[calc(100vh-260px)]">
          <Card>
            <DatasetSelectWithPopover
              ...
              buttonText={
                selectedDatasets.length ? "1 selected" : "Select Dataset"
              }
            />
          </Card>
          ...
          <Card className="flex-1 min-h-[300px] flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                <NumericInput
                  ...
                  onValueChange={(valueAsNumber) =>
                    setPatchWidth(valueAsNumber || DEFAULT_PATCH_SIZE)
                  }
                ...
                <NumericInput
                  ...
                  value={patchHeight}
                  onValueChange={(valueAsNumber) =>
                    setPatchHeight(valueAsNumber || DEFAULT_PATCH_SIZE)
                  }
                  disabled={!datasetId || loadingImages}
                  className="numeric-input"
                />
            ...
            {(annotationSetChannelOptions.length > 0 ||
              selectedAnnotationChannel !== "") && (
              <div className="flex flex-col gap-3 mb-3">
                <HTMLSelect
                  ...
                  onChange={(event) =>
                    handleSelectedAnnotationChannelChange(event.target.value)
                  }
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <NumericInput
                          ...
                          onValueChange={(valueAsNumber) =>
                            handleAnnotationSetChannelScaleChange(
                              selectedAnnotationChannel,
                              "min",
                              valueAsNumber,
                            )
            ...
            <div className="flex flex-col gap-3">
              <div className="flex gap-2 items-center">
                <HTMLSelect
                  ...
                  onChange={(event) =>
                    setSelectedAnnotationSetId(event.target.value)
                  }
                ...
                >
                  ...
                  {annotationSets.map((annotationSet) => (
                    <option key={annotationSet.id} value={annotationSet.id}>
                      {annotationSet.name ||
                        `Annotation set ${annotationSet.id}`}
                    </option>
                  ))}
                </HTMLSelect>
              ...
              <textarea
                ...
                onChange={(event) =>
                  setAnnotationSetDescription(event.target.value)
                }
                disabled={!datasetId || loadingAnns}
              />
            ...
            {loadingAnns ? (
              ...
              <AnnotationViewer
                ...
                onChannelVisibilityChange={
                  handleCurrentImageChannelVisibilityChange
                }
                ...
                normalizationChannelScales={
                  currentDatasetNormalization.channelScales
                }
                lockedChannelIndex={
                  selectedAnnotationChannel !== ""
                    ? selectedAnnotationChannel
